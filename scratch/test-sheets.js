...
async function test() {
  ...
  // Get spreadsheet headers and values
  const url = `${base}/values/${encodeURIComponent(SHEET_PATIO)}!A1:T20`;
  const resp = await fetch(url, {
    headers: { Authorization: `Bearer ${token}` }
  });
  const data = await resp.json();
  console.log("Sheet Headers and some rows:", JSON.stringify(data, null, 2));
}
