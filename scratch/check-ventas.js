...
async function test() {
  ...
  // Find which sheet has sales. It is probably called 'Ventas' or similar.
  console.log("Sheets in document:");
  for (const s of doc.sheetsByIndex) {
    console.log(`- ${s.title}`);
  }
  
  const sheet = doc.sheetsByTitle['Ventas_2026'] || doc.sheetsByTitle['Ventas'] || doc.sheetsByIndex[0];
  console.log(`Reading from sheet: ${sheet.title}`);
  const rows = await sheet.getRows();
  console.log(`Total rows in ${sheet.title}: ${rows.length}`);
  
  // Print last 5 sales
  const lastRows = rows.slice(-5);
  for (let i = 0; i < lastRows.length; i++) {
    console.log(`Sale ${i}:`, lastRows[i].toObject());
  }
}
