const { GoogleAuth } = require('google-auth-library');
const { google } = require('googleapis');

const SPREADSHEET_ID = '1V6EmrQQIExA3UtAUeJsdAZESa1S5WiGQRAOsfHsQ6E8';

const CREDENTIALS = {
  type: 'service_account',
  project_id: 'gowash-db-496413',
  private_key_id: 'f6d0c0f05f12ea9b752d05db3b38ef336a4cc735',
  private_key: '-----BEGIN PRIVATE KEY-----\\nJ1Rmx+e/HDerkKUnWFTDj6IR\n-----END PRIVATE KEY-----\n',
  client_email: 'gowash-sync@gowash-db-496413.iam.gserviceaccount.com',
  client_id: '107095811744069109144',
};

async function main() {
  const auth = new GoogleAuth({
    credentials: CREDENTIALS,
    scopes: ['https://www.googleapis.com/auth/spreadsheets'],
  });

  const sheets = google.sheets({ version: 'v4', auth });

  try {
    // 1. Check if 'Servicios' sheet exists
    const spreadsheet = await sheets.spreadsheets.get({
      spreadsheetId: SPREADSHEET_ID,
    });
    
    const sheetExists = spreadsheet.data.sheets.some(
      s => s.properties.title === 'Servicios'
    );

    if (!sheetExists) {
      console.log('Creating Servicios sheet...');
      await sheets.spreadsheets.batchUpdate({
        spreadsheetId: SPREADSHEET_ID,
        requestBody: {
          requests: [{
            addSheet: {
              properties: {
                title: 'Servicios',
              }
            }
          }]
        }
      });
    }

    // 2. Populate data
    console.log('Populating data...');
    const values = [
      ['Nombre del Servicio', 'Precio', 'Descripcion', 'Tiempo Estimado (min)'],
      ['M', 25000, 'Lavado M', 30],
      ['L', 28000, 'Lavado L', 35],
      ['XL', 30000, 'Lavado XL', 40],
      ['XXL', 35000, 'Lavado XXL', 45],
      ['XXXL', 40000, 'Lavado XXXL', 50],
      ['CXL', 40000, 'Lavado CXL', 50],
      ['CXXL', 45000, 'Lavado CXXL', 60],
      ['Moto (250cc)', 22000, 'Lavado Moto', 25],
      ['Moto Grande', 28000, 'Lavado Moto Grande', 30]
    ];

    await sheets.spreadsheets.values.update({
      spreadsheetId: SPREADSHEET_ID,
      range: 'Servicios!A1:D10',
      valueInputOption: 'RAW',
      requestBody: {
        values: values,
      },
    });

    console.log('Servicios sheet updated successfully!');
  } catch (error) {
    console.error('Error:', error);
  }
}

main();
